...
5. **Branch с хотя бы одним дочерним узлом не может быть удалён** →
   ...
   добавляет `errors[:base]` с точным сообщением «Branch has children
...
- **Ответ `409 Conflict`:** у branch есть хотя бы один дочерний узел.
  Тело: `{ "error": ["Branch has children and cannot be deleted"] }`
...
| E25 | `destroy` узла с хотя бы одним ребёнком | 409 | `{"error":["Branch has children and cannot be deleted"]}` |
...
`{"error":["Branch has children and cannot be deleted"]}`. A остался
...
`before_destroy` callback с точным сообщением «Branch has children
