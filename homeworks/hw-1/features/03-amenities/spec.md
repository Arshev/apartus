...
5. Для `create` (attach):
   ...
   - Если `amenity_id` в permitted-параметрах отсутствует (wrapper
     есть, поле нет) — `find_by(id: nil)` → `nil` → модель
     `unit_amenities.new(amenity_id: nil)` → валидация `Amenity must
     exist` → `422`. Это сознательное поведение (см. E22b в §9).
...
| E22b | `attach` с `{"unit_amenity": {}}` (wrapper есть, `amenity_id` нет) | 422 | `{"error":["Amenity must exist"]}` |
