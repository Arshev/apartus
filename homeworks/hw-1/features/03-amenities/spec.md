...
| E22a | `attach` с `{"unit_amenity": {}}` (пустой wrapper) | 400 | ParameterMissing (Rails `require` считает пустой хэш blank) |
| E22b | `attach` с `{"unit_amenity": {"ignored":"x"}}` (wrapper с другими полями, без `amenity_id`) | 404 | — (единое 404 с E20/E21, §5.6) |
