...
- `branch_id` — nullable integer, добавлен в F5. Null для Property,
  не привязанных к филиалу.
...
соответствуют, `property_type` всегда строка. После F5
retrospective update набор ключей расширен одним nullable integer
полем `branch_id` (§6 и §13 retrospective note).
...
Поле `branch_id` добавлено Spec F5 (nullable integer FK на
`branches`). F1 JSON-контракт §6 расширен одним nullable ключом,
AC9 обновлён. F1 existing PropertiesController и тесты
`properties_spec.rb` получили минимальные изменения:

- `permit` расширен на `:branch_id`.
- `property_json` возвращает `branch_id`.
- Security-критический resolve через `Current.organization.branches`
  с sentinel pattern F4 — см. F5 Spec §4.2 ANTI-PATTERN.
- Test `expected_keys` массив включает `branch_id`.

Это изменение — прецедент SDD: F5 owns retrospective patches
(DEC-015). Старый текст F1 §6 / AC9 заменён актуальным, а не
«замаркирован как устаревший».
