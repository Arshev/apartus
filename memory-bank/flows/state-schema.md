...
2. **Phase меняется только при прохождении gate.** Gate-предикаты — в [feature-flow.md](feature-flow.md) секция "Gates". Выполняется через `/phase-advance` (или ручной правкой при необходимости).
...
7. **Blocker добавляется сразу при обнаружении** через `/phase-block` (или ручной правкой).
