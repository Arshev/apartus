...
- `REQ-04` **Price breakdown panel.** Extract `ReservationPriceSummary.vue`. Props: `{ checkIn, checkOut, unitId, basePriceCents, seasonalPrices, currency, manualTotalCents, autoTotalCents }`. Renders:
...
- `REQ-05` **Guest quick-create dialog.** Extract `GuestQuickCreateDialog.vue`. Compact variant `GuestFormView` (first_name, last_name required; email, phone optional; notes omitted). Triggered by `v-btn icon="mdi-plus"` next to guest combobox. On successful create — emit `created(guest)`, parent appends to guests list и устанавливает `form.guest_id`. On error — dialog stays open с inline alert, parent form untouched.
...
- `REQ-09` **Edit mode prefill.** GET `/reservations/:id` returns `total_price_cents` — prefill напрямую (без `/100` conversion), `manualOverride = true` initially (чтобы не перезаписать existing price при first watcher fire).
...
- `REQ-15` **Tests.**
  - `ReservationPriceSummary.test.js` — breakdown pure-function (base only, 1 seasonal, 2 seasonals straddling, empty state, diff chip visibility)
  - `ReservationDateRangePicker.test.js` — display format, nights count, menu open/close, range emit
  - `GuestQuickCreateDialog.test.js` — submit success emits created; error stays open
  - `ReservationFormView.test.js` (expand existing) — prefill uses cents; manual override locks auto-calc; submit payload `total_price_cents`; currency from authStore; guest dialog flow
...
- `ASM-04` Vuetify 4 `v-date-picker` supports `multiple="range"` prop; emits array of dates (start..end inclusive, или [start, end]). Версия проверяется перед имплементацией.
...
│     <v-combobox guest />  [+ Новый гость button]
...
- `FM-01` **Vuetify range emit shape drift** — `v-date-picker multiple="range"` may emit full array of dates or just [start, end] depending on version. Mitigation: normalize в DateRangePicker (`checkIn = range[0]`, `checkOut = range[range.length - 1]` after `+1 day` if exclusive). Verify in implementation.
...
| `REQ-12` | i18n | `EC-07` | `CHK-04` | — |
| `REQ-13` | solution | `SC-10` | `CHK-05` | `EVID-05` |
...
| `CHK-03` | — | diff stat | ~600 LoC added (components + tests) | — |
