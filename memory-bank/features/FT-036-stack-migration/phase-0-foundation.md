...
- `REQ-03` **Tailwind configuration.** Create `frontend/src/styles/tailwind.css` entrypoint with:
  - `@import "tailwindcss"`
  - `@theme` block с colors (OKLCH palette port), fontFamily (Geologica/Geist/mono), spacing extensions if any
  - Dark mode via `@media (prefers-color-scheme: dark)` fallback + `.dark` class variant
  - Import в `main.js` (before Vuetify CSS чтобы Vuetify смог override где overlap)
...
- `REQ-07` **Dark mode store hookup.** Ensure existing theme toggle (wherever в Settings/Topbar) calls `document.documentElement.classList.toggle('dark')` — this triggers both Tailwind `dark:*` utilities и PrimeVue `darkModeSelector`. Verify Vuetify theme toggle also still works — либо align in P0, либо оставить Vuetify-only toggle до P1.
- `REQ-08` **Test helper.** Create `frontend/src/__tests__/helpers/mountWithPrimeVue.js`. Options:
  - A. Install PrimeVue plugin в test app + real components (if jsdom handles them)
  - B. Simple wrapper с plugins array [pinia, router, i18n, PrimeVue] без stubs
  - If some components require stubs (Menu, OverlayPanel), add minimally
  Keep existing `mountWithVuetify` working для hybrid tests.
- `REQ-09` **Smoke conversion.** Convert `src/views/PlaceholderView.vue` — trivial view — to PrimeVue + Tailwind. Test passes with `mountWithPrimeVue`. Demonstrates end-to-end pipeline works.
...
- `REQ-11` **Icons.** PrimeIcons installed, `primeicons/primeicons.css` imported. MDI icons НЕ удаляются в P0 (used в Vuetify views). Migration per-view в P1..P6.
- `REQ-12` **Coverage ratchet.** After P0 `yarn test:coverage` meets ≥ 94%. Baseline preserved.
- `REQ-13` **Build sanity.** `yarn build` success. Bundle size change recorded (will grow temporarily due to dual stack).
- `REQ-14` **Phase commits.** Logical commit sequence:
  1. Install deps (package.json + yarn.lock)
  2. Tailwind setup (vite.config + tailwind.css)
  3. PrimeVue plugin + theme port
  4. main.js integration
  5. Test helper
  6. PlaceholderView smoke conversion
  7. Register FT-036 in features/README.md
...
- `EC-05` `yarn test:coverage --run` — ≥ 94%.
